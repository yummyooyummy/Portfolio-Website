    ...
    work: {
      ...
      projects: [
        ...
        },
        {
          ...
          cardTitle: "AI 生成之后的那一步：按你定的标准，逐层验收设计稿",
          ...
          sections: [
            ...
            { heading: "边用边调：三轮把误报清零", body: "规则不是一次想清楚的，是拿 AI 生成的真实文件一轮轮试出来的。\n\n第一版 28 个问题，19 个是误报——文字外面套的同名 Frame 被判\"不是文字图层\"，emoji 图标被要求改成 txt/。第二版 9 个问题，全是误报。第三版归零。根因是同一个：我在做**字符串匹配**，而正确的做法是**先看这个图层实际是什么**。\n\n每一轮都是同样的动作：拿真实稿子跑一遍 → 看哪条报错不合理 → 改规则 → 再跑。**工具好不好用是磨出来的，不是一次设计出来的。**" },
            { heading: "两级结果：必须修的，和值得看的", body: "结果分两级：**必须修的**是客观违规，计入合格率；**建议看看的**是启发式判断，不计分。\n\n同一套界面设计，Figma Make 原始产出 88 层、88 个必须修；把命名规范交给 AI 重新生成后 0 个必须修，但还剩 24 条建议——**AI 会严格遵守你写下的规则，但不会做你没说的事**，那 24 条全是规范里没写的两件事。\n\n所以哪怕合格率 100%，那 24 条也值得看一眼。**机器能判的是\"有没有违规\"，判不了的是\"这样做是不是更好\"——把后者留成建议交回给人，比强行计分更诚实。**", images: [{ src: "/assetgate-test.png", label: "Result · 两份文件的检查结果", alt: "两份文件的检查结果对比：原始产出 88 层、88 个必须修；按规范重新生成后 0 个必须修、24 条建议", wide: true }], imageLayout: "sketch" },
            { heading: "反思与下一步", body: "如果重来，我会把标准**前置到生成环节**：在提示里就注入命名与结构要求，让 AI 生成时就合规，检查器只做兜底。事后拦截，永远比事前约束贵。\n\n更完整的形态是把链路打通：**生成时注入标准 → 自动整理 → 机器验收 → 合格资产打标入库。** 现在 AssetGate 只占第三环。下一步接上 Figma Agent，等链路闭合，它就不只是一个插件——**这也是我理解的 AI 时代设计工具该走的方向：不是替设计师做决定，而是把设计师定下的标准，变成机器可以反复执行的东西。**" },
          ]
        },
        {
          slug: "genesis",
    ...
    work: {
      ...
      projects: [
        ...
        },
        {
          ...
          cardTitle: "The step after AI generates: checking the file layer by layer against your standard",
          ...
          sections: [
            ...
            { heading: "Three design decisions", body: "**Trust first.** Every issue shows the exact standard it violates, clicking one jumps to that layer, and the panel permanently states that it only inspects and never edits. The five things a machine cannot judge are spelled out at the top rather than pretended away.\n\n**The standards belong to the user.** I built the ability to define a convention and enforce it, not one particular convention. Every setting is a plain-language checkbox: no regex, no docs.\n\n**Only rules a machine can judge objectively.** Definite violations count toward the pass rate; heuristic ones are suggestions only. Better to under-report than misreport.", images: [{ src: "/assetgate-ui.png", label: "UI · Setup and results", alt: "The two AssetGate views: setup with plain-language checkboxes and the five things it cannot judge stated at the top; results with the pass rate and issues grouped by standard", wide: true }], imageLayout: "sketch" },
            { heading: "Tuned by use: three rounds to zero false positives", body: "The rules weren't thought through once — they were tested out on real AI-generated files, round after round.\n\nFirst version: 28 findings, 19 of them false — a same-named Frame wrapping text was flagged as not a text layer, an emoji used as an icon was told to become txt/. Second version: 9 findings, all false. Third: zero. Same root cause every round: I was **matching strings** when the right approach was to **look at what the layer actually is**.\n\nEvery round was the same loop: run it on a real file, find the finding that doesn't hold up, fix the rule, run again. **A tool becomes usable by being ground down like this, not by being designed right the first time.**" },
            { heading: "Two tiers: must-fix, and worth a look", body: "Findings come in two tiers: **must-fix** are objective violations and count toward the pass rate; **worth a look** are heuristic and don't score.\n\nOn the same screen design, Figma Make's raw output gave 88 layers and 88 must-fix; handing the naming spec to the AI and regenerating gave zero must-fix — but 24 suggestions remained. **AI follows the rules you write down and does nothing about the ones you don't**; all 24 were things the spec never mentioned.\n\nSo even at a 100% pass rate, those 24 are worth reading. **A machine can judge whether something breaks a rule; it can't judge whether something would be better — handing that second question back to a person as a suggestion is more honest than forcing it into a score.**", images: [{ src: "/assetgate-test.png", label: "Result · Two files compared", alt: "Two files compared: raw output at 88 layers with 88 must-fix; the same design regenerated to spec with 0 must-fix and 24 suggestions", wide: true }], imageLayout: "sketch" },
            { heading: "Reflection and next step", body: "If I did it again, I'd move the standard **upstream into generation**: inject the naming and structural requirements into the prompt so the output is compliant as it's produced, with the linter as a backstop. Catching things afterwards always costs more than constraining them up front.\n\nThe fuller shape is a closed loop: **inject the standard at generation → tidy automatically → machine sign-off → tag compliant assets into the library.** AssetGate covers only the third link today. Next is wiring in a Figma Agent so the tidying runs on its own — once the loop closes, it stops being just a plugin. **That's where I think design tooling goes in the AI era: not making decisions for designers, but turning the standards designers set into something a machine can execute over and over.**" },
          ]
        },
        {
          slug: "genesis",
