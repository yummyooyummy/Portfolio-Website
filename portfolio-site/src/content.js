export const content = {
  zh: {
    ...
    about: {
      ...
      experiences: [
        ...
        },
        {
          ...
          desc: "负责大型开放世界游戏的战斗 HUD，副本、地图、家园、宠物等核心玩法系统的交互设计，以及手机、PC、主机三端的操作设计。从承接需求到独立主导核心模块、推动跨部门对齐与落地，完整走过了大厂产品协作的全流程。"
        },
        {
          org: "北京大学信息技术高等研究院（杭州）",
          role: "交互设计实习生",
          period: "2021.05 – 2021.10",
          desc: "参与 B 端数字大屏的数据可视化项目，把复杂的工业生产数据转化为清晰可读的界面，完成从需求梳理、信息架构到界面落地的全过程。"
        }
      ],
      educationTitle: "教育背景",
      educationIntro: "我的设计之路从纯艺术开始——四年工作室艺术的训练，给了我对形式与美感近乎本能的敏锐；交互设计再把这种敏锐收进理性的框架。既懂什么是美，也懂怎么让它真正好用。",
      education: [
        {
          degree: "硕士",
          school: "悉尼科技大学",
      ...
      projects: [
        {
          ...
          nda: "本页仅使用游戏官方公开素材与实机画面；受保密协议限制，不展示内部设计稿与过程物料，完整案例可在面试中详细讲解。",
          ...
          sections: [
            { heading: "战斗中的实时交互", body: "战斗 HUD 是交互设计里最严苛的场景：玩家的注意力在敌人身上，界面只能用余光阅读。我负责血条、技能盘、战斗状态信息的整体交互设计——哪些信息常驻、哪些事件触发、哪些必须强提醒，本质是在**为玩家的注意力做预算**：每一个像素的信息，都要回答\"此刻玩家需要它吗\"。\n\n伤害跳字是其中我从头负责的模块。跳字要在高频刷屏下保持可读，不同伤害类型要一眼可辨，同时还要给操作以\"打中了\"的确认感——可读性、辨识度、反馈感三者的平衡，我反复调到帧级。", images: [{ src: "/hok-hud.jpg", alt: "《王者荣耀·世界》实机战斗画面：HUD、技能盘与伤害跳字（官方公开内容）" }, { src: "/hok-party.jpg", alt: "《王者荣耀·世界》实机画面：多人组队 HUD 与目标指引（官方公开内容）" }] },
            { heading: "玩法系统的交互设计", body: "副本、地图、家园、宠物、成就、钓鱼——每接一个系统，我的工作都是同一个完整循环：吃透玩法规则和数值逻辑，梳理信息架构和玩家的核心路径，设计交互流程和界面框架，穷举状态与边界情况，再输出交互文档推动落地。\n\n数年间一个接一个做下来，这套**\"把复杂翻译成清晰\"**的方法已经成为肌肉记忆——而这套方法不属于游戏，它属于任何复杂产品。", images: [{ src: "/hok-dungeon.jpg", alt: "《王者荣耀·世界》实机画面：副本玩法界面（官方公开内容）" }, { src: "/hok-pet.jpg", alt: "《王者荣耀·世界》实机画面：宠物系统界面（官方公开内容）" }] },
        ...
        },
        {
          ...
          sections: [
            { heading: "起点：结构化缺口", body: "AI 让\"画出一版界面\"变得很快，但产出的文件是非结构化的：图层叫 Frame 1、Container、Text，层级随手嵌套。人能看，机器不能用——无法批量导出、交接或沉淀为可复用资产。\n\n**当执行不再是瓶颈，产出物的结构化质量就成了新瓶颈。**" },
            { heading: "定位：管线上的质量门", body: "动手前先做了一轮市场调研。现有 Figma 插件集中在两类：**改名**（Figma 内置 AI 重命名、Name Lint、Layeria 等）和**样式一致性**（Design Lint 等）。没有一个是按团队自定义的规则、同时校验命名与结构、并明确区分\"必须修\"与\"建议\"的验收工具——尤其没有针对 AI 生成稿的。\n\nAssetGate 补的是这个位置：**AI 生成 → AI 按规范整理 → AssetGate 验收。** 标准写一次，机器执行一万次，返工发生在交接之前。", images: [{ src: "/assetgate-flow.png", label: "Flow · 有无质量门", alt: "有无质量门的两条工作流对比：没有它，问题在交接后才暴露；有它，不合格的稿子在交接前被打回整理", wide: true }], imageLayout: "sketch" },
            { heading: "三个设计决策", body: "**信任优先。** 每条问题回显它违反的具体标准，点击即跳转定位，界面常驻\"只检查，不改动文件\"；机器判不了的五类情况写在面板最顶部，不假装能判。\n\n**标准由使用者定义。** 做的是\"定义规范 + 执行检查\"的能力，不是某一套具体规范。设定项全是大白话勾选，不需要懂正则。\n\n**只做客观可判的规则。** 确定违规的计入合格率，启发式判断的只算建议——宁可少报，不可误报。", images: [{ src: "/assetgate-ui.png", label: "UI · 设定区与结果区", alt: "AssetGate 插件的两个界面：设定区用大白话勾选标准、顶部明示五类判不了的情况；结果区显示合格率与按标准分组的问题清单", wide: true }], imageLayout: "sketch" },
            { heading: "用真实文件校准规则", body: "第一版跑在自己的文件上：28 个问题，19 个误报——文字外层的同名 Frame 被判\"不是文字\"，emoji 图标被要求改成 txt/。第二版：9 个问题，全部误报——组件式命名被判\"应为中文\"，有填充的分割线被判\"空图层\"。第三版归零。\n\n三轮同一根因：我在做**字符串匹配**，而正确做法是**先看图层实际是什么**。用户就是我自己，数据没有客气。" },
            { heading: "对照测试：0% → 100%", body: "同一套界面设计，两份文件。Figma Make 原始产出 88 层、88 个问题；把命名规范交给 AI 重新生成后 87 层、0 个必须修，剩 24 条结构类建议。\n\n**AI 会严格遵守你写下的规则，但不会做你没说的事。** 那 24 条全是规范里没写的两件事：多余的包装层、该组件化的重复元素。", images: [{ src: "/assetgate-test.png", label: "Test · 原始稿 vs 规范稿", alt: "对照测试：Figma Make 原始产出合格率 0%（88 层、88 个必须修）；同一设计按规范重生成后 100%（87 层、0 个必须修、24 条建议）", wide: true }], imageLayout: "sketch" },
            { heading: "反思与下一步", body: "如果重来，我会把标准**前置到生成环节**：在提示里注入命名与结构要求，让 AI 生成时就合规，检查器只做兜底。\n\n下一步是接上 Figma Agent，让\"AI 整理\"也自动运行——那时 AssetGate 才真正成为一条自动化管线上的门，而不只是一个插件。" }
          ]
        },
        {
          slug: "genesis",
    ...
    ai: {
      ...
      intro: [
        "AI 让**执行的成本无限趋近于零**——一个想法，几小时内就能变成可用的产品。但当执行不再是瓶颈，判断就成了真正稀缺的东西：AI 能生成无数个方案，却分不清哪一个是对的。",
        "这正是我的位置。多年的交互设计训练，让我对\"好\"有清晰的标准，能在 AI 大量的产出里**快速筛掉平庸的、留下对的**，并把它打磨到我满意的质感。",
      ...
      workflow: {
        ...
        roleLabel: "我的角色：",
        stages: [
          { num: "01", title: "洞察", doing: "动手之前，先用 AI 把领域快速摸透——市场、竞品、真实痛点。调研从几天压缩到几小时，但分出真问题和噪音，靠的还是设计师的眼睛。", role: "分辨真正值得解决的问题。" },
          { num: "02", title: "构想", doing: "和 AI 充分发散，把模糊的念头展开成多个方向，预判风险与可行性，再收敛出值得做的那一个。", role: "决定做什么，以及为什么做。" },
    ...
    contact: {
      ...
      label: "联系",
      heading: "一起做点东西",
      intro: "我正在寻找下一个机会——一个能把交互设计的深度，和借助 AI 独立做出产品的能力，都用上的地方。比起被动接收需求，我更习惯主动把问题想透、推进到结果。如果你在找一位既能深入设计、也能独立落地的设计师，欢迎与我聊聊。",
      status: "正在看新机会",
  ...
  en: {
    ...
    about: {
      ...
      experiences: [
        ...
        },
        {
          ...
          desc: "Owned the battle HUD, the interaction design of core gameplay systems — dungeons, map, home, pets — and input design across mobile, PC, and console for a large-scale open-world game. From taking requirements to independently leading core modules and driving cross-team delivery, I went through the full product-collaboration cycle inside a major organization."
        },
        {
          org: "Peking University IT Institute (Hangzhou)",
          role: "Interaction Design Intern",
          period: "May 2021 – Oct 2021",
          desc: "Contributed to a B-end data-visualization project for large-screen displays, translating complex industrial data into clear, readable interfaces — from requirements and information architecture to final UI."
        }
      ],
      educationTitle: "Education",
      educationIntro: "My design path started with pure art — four years of studio art gave me an instinctive sensitivity to form and beauty; interaction design then channeled it into a rational framework. I understand what's beautiful, and how to make it truly usable.",
      education: [
        {
          degree: "M.A.",
      ...
      projects: [
        {
          ...
          nda: "This page uses only officially released material and live gameplay footage. Due to NDA, internal design files and process work are not shown; the full case can be discussed in detail in an interview.",
          ...
          sections: [
            { heading: "Real-time interaction in combat", body: "The battle HUD is one of the harshest environments in interaction design: the player's attention is on the enemy, and the interface can only be read with peripheral vision. I owned the interaction design of health bars, the skill wheel, and combat status information — deciding what stays persistent, what appears on events, and what demands a hard interrupt. It is, at heart, **budgeting the player's attention**: every pixel of information has to answer \"does the player need this right now?\"\n\nDamage numbers were a module I owned end to end. They must stay readable under rapid-fire spam, different damage types must be distinguishable at a glance, and every hit still needs to feel confirmed. Balancing readability, distinction, and feedback — I tuned it frame by frame.", images: [{ src: "/hok-hud.jpg", alt: "Honor of Kings: World live combat — HUD, skill wheel, and damage numbers (official public content)" }, { src: "/hok-party.jpg", alt: "Honor of Kings: World live gameplay — party HUD and objective guidance (official public content)" }] },
        ...
        },
        {
          ...
          sections: [
            { heading: "Starting point: the structure gap", body: "AI made \"draw a version of this screen\" fast, but the output is unstructured: layers named Frame 1, Container, Text; nesting arbitrary. Readable by people, useless to machines — it can't be batch-exported, handed off, or turned into reusable assets.\n\n**Once execution stops being the bottleneck, the structural quality of the output becomes the new one.**" },
            { heading: "Positioning: a gate on the pipeline", body: "I researched the market before building. Existing Figma plugins cluster in two groups: **renaming** (Figma's built-in AI rename, Name Lint, Layeria and others) and **style consistency** (Design Lint and the like). None validate naming and structure together against team-defined rules, separate must-fix from suggestion, and act as a sign-off step — least of all for AI-generated files.\n\nThat's the slot AssetGate fills: **AI generates → AI tidies to spec → AssetGate signs off.** Write the standard once, let a machine run it ten thousand times, and catch rework before hand-off instead of after.", images: [{ src: "/assetgate-flow.png", label: "Flow · With and without a gate", alt: "Two workflows compared: without a gate, problems surface after hand-off; with AssetGate, non-compliant files are sent back for tidying before hand-off", wide: true }], imageLayout: "sketch" },
            { heading: "Three design decisions", body: "**Trust first.** Every issue shows the exact standard it violates, clicking jumps to the layer, and the panel permanently states it only inspects, never edits. The five things a machine can't judge are listed at the very top rather than pretended away.\n\n**Standards belong to the user.** I built the ability to define a convention and enforce it, not one particular convention. Every setting is a plain-language checkbox — no regex.\n\n**Only objectively judgeable rules.** Definite violations count toward the pass rate; heuristics are suggestions only. Better to under-report than misreport.", images: [{ src: "/assetgate-ui.png", label: "UI · Setup and results", alt: "The two AssetGate views: setup with plain-language checkboxes and the five things it cannot judge stated at the top; results with the pass rate and issues grouped by standard", wide: true }], imageLayout: "sketch" },
            { heading: "Calibrating the rules on real files", body: "Version one, run on my own files: 28 issues, 19 false positives — a same-named Frame around each text was flagged as \"not a text layer\", emoji icons were told to become txt/. Version two: 9 issues, all false — component-style names flagged as \"should be Chinese\", a filled divider flagged as \"empty\". Version three: zero.\n\nOne root cause across all three rounds: I was **matching strings** when I should have been **checking what the layer actually is**. The user was me, and the data didn't spare my feelings." },
            { heading: "Controlled test: 0% → 100%", body: "Same screen design, two files. Figma Make's raw output: 88 layers, 88 issues. After handing the naming spec to the AI and regenerating: 87 layers, zero must-fix, 24 structural suggestions left.\n\n**AI follows the rules you write down and does nothing about the ones you don't.** All 24 were the two things the spec never mentioned: redundant wrapper layers and repeats that should be components.", images: [{ src: "/assetgate-test.png", label: "Test · Raw vs to-spec", alt: "Controlled test: Figma Make raw output at 0% (88 layers, 88 must-fix); the same design regenerated to spec at 100% (87 layers, 0 must-fix, 24 suggestions)", wide: true }], imageLayout: "sketch" },
            { heading: "Reflection and next step", body: "If I did it again I'd move the standard **upstream into generation**: inject naming and structure requirements into the prompt so output is compliant at birth, with the linter as a backstop.\n\nNext: wire in a Figma Agent so the \"AI tidies\" step runs on its own. That's when AssetGate becomes a gate on an automated pipeline, not just a plugin." }
          ]
        },
        {
          slug: "genesis",
    ...
    ai: {
      ...
      workflow: {
        ...
        roleLabel: "My role: ",
        stages: [
          { num: "01", title: "Discover", doing: "Before building, I use AI to map a field fast — market, competitors, real pain points. Research shrinks from days to hours, but telling real problems from noise still takes a designer's eye.", role: "Tell which problems are worth solving." },
          { num: "02", title: "Ideate", doing: "Diverge widely with AI, open a vague notion into multiple directions, gauge risk and feasibility, then converge on the one worth doing.", role: "Decide what to build, and why." },
          { num: "03", title: "Define", doing: "With AI I produce two key documents: a product design doc that pins down features and boundaries, and a development plan that breaks the design into executable steps.", role: "Turn a fuzzy idea into a clear spec." },
          { num: "04", title: "Design", doing: "Interfaces and interactions designed primarily in Figma, with Claude and Midjourney woven in to iterate fast. Tools combine as needed; quality stays my call.", role: "Own the quality of every screen." },
          { num: "05", title: "Build", doing: "Claude Code turns the designs into real code. I test screen by screen, log issues, give precise directions, until what's built matches what was designed.", role: "Review every detail like a delivery." },
          { num: "06", title: "Refine", doing: "Ship a minimum usable version first, then refine through real use — filling in details, growing \"usable\" into \"mature.\"", role: "Grow a product from usable to mature." }
        ]
      },
      lessons: {
    ...
    contact: {
      ...
      intro: "I'm looking for my next opportunity — a place that calls on both the depth of interaction design and the ability to independently build products with AI. Rather than waiting for requirements, I tend to think problems through and push toward real results. If you're looking for a designer who can both go deep and ship independently, I'd love to talk.",
